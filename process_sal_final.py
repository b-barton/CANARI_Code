...
with nc.Dataset(flist_s[0], 'r') as nc_fid:
    ...
    sal = nc_fid.variables[v_map['sal']][:]
...
print(x1, x2, lon.shape)

lat = lat[y1:y2:sub, x1:x2:sub]
lon = lon[y1:y2:sub, x1:x2:sub]
tmask = (sal[:, 0, y1:y2:sub, x1:x2:sub] == 1e20) | (tmask[:, 0, y1:y2:sub, x1:x2:sub] == 0)
...
def dask_load_s(fns, tmask, y1, y2, x1, x2, sub):
    ...
    lazy_arrays = [dask.array.from_delayed(
                        x.variables[v_map['sal']][:, 0, y1:y2:sub, x1:x2:sub], shape=sal[:, 0, y1:y2:sub, x1:x2:sub].shape, dtype=np.float64) 
...
for i in range(len(flist_s) // 12):
    ...
    sal_part = (
        ...
        ds_s[:, :, :, np.newaxis].repeat(3, axis=3)) # t, y, x, mask
        .mean(axis=(1, 2)) # t, mask
        )
    print(date[i1])
    print(sal_part)
    sal_time[i1:i2, :] = sal_part.compute()
