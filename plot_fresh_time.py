...
data = np.load(out_file + 'freshwater_content.npz', allow_pickle=True)
fresh_time = data['fresh_time']
date = data['date']
data.close()

fresh_time = np.ma.masked_where(fresh_time==-1e20, fresh_time)

data = np.load(out_file + 'fresh_mn_1990-2000.npz', allow_pickle=True)
fresh_map1 = data['fresh_map']
lat = data['lat']
lon = data['lon']
data.close()

fresh_map1 = np.ma.masked_where(fresh_map1==-1e20, fresh_map1)
fresh_mean1= np.ma.mean(fresh_map1, axis=0)
fresh_std1= np.ma.std(fresh_map1, axis=0)

data = np.load(out_file + 'fresh_mn_2040-2050.npz', allow_pickle=True)
fresh_map2 = data['fresh_map']
lat = data['lat']
lon = data['lon']
data.close()

fresh_map2 = np.ma.masked_where(fresh_map2==-1e20, fresh_map2)
fresh_mean2 = np.ma.mean(fresh_map2, axis=0)
fresh_std2 = np.ma.std(fresh_map2, axis=0)
...
run = 12
fresh_run = np.ma.zeros((fresh_time.shape[0] - run, fresh_time.shape[1]))
date_run = np.zeros((len(plot_date) - run), dtype=object)

for i in range(len(date_run)):
    fresh_run[i] = np.ma.mean(fresh_time[i:i + run, :], axis=0)
...
cs1 = ax1.pcolormesh(lon, lat, fresh_mean1, transform=data_crs, cmap=my_cm, vmin=-10, vmax=30)
cs2 = ax2.pcolormesh(lon, lat, fresh_mean2 - fresh_mean1, transform=data_crs, cmap=my_cm, vmin=-10, vmax=10)

cs3 = ax3.pcolormesh(lon, lat, fresh_std1, transform=data_crs, cmap=my_cm, vmin=-1, vmax=1)
cs4 = ax4.pcolormesh(lon, lat, fresh_std2 - fresh_std1, transform=data_crs, cmap=my_cm, vmin=-1, vmax=1)

ax5.plot(plot_date, fresh_time[:, 0], color='tab:blue', alpha=0.5)
ax5.plot(plot_date, fresh_time[:, 1], color='tab:orange', alpha=0.5)
ax5.plot(plot_date, fresh_time[:, 2], color='tab:green', alpha=0.5)

ax5.plot(date_run, fresh_run[:, 0], color='tab:blue', label='> 70 N')
ax5.plot(date_run, fresh_run[:, 1], color='tab:orange', label='> 75 N')
ax5.plot(date_run, fresh_run[:, 2], color='tab:green', label='> 80 N')
...
ax5.set_ylim([0.4e14, 1.3e14])
ax5.set_xlim([dt.datetime(1950, 1, 1), dt.datetime(2051, 1, 1)])
ax5.legend(loc='lower right')
ax5.set_ylabel('Freshwater Content (m$^{3}$)')
...
cax1.set_xlabel('Freshwater Content (m)')

fig1.colorbar(cs2, cax=cax2, orientation='horizontal')
cax2.set_xlabel('Diff. Freshwater Content (m)')

fig1.colorbar(cs3, cax=cax3, orientation='horizontal')
cax3.set_xlabel('Freshwater Content StD. (m)')

fig1.colorbar(cs4, cax=cax4, orientation='horizontal')
cax4.set_xlabel('Diff. Freshwater Content StD. (m)')


# In[ ]:


fig1.savefig('./Figures/fresh_content_time.png')
