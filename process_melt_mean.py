...
fn_nemo_dat_s1 = make_path('siflfwbot', i_o, freq, 'hist-1950')
fn_nemo_dat_s2 = make_path('siflfwbot', i_o, freq, time_s)
...
melt_time = np.ma.zeros((12, lon.shape[0], lon.shape[1]))
for m in range(1, 13):
    for y in yr_uni:
        ind = np.nonzero(yr == y)[0][0] + date_use[0]
        with nc.Dataset(flist_s[ind], 'r') as nc_fid:
            melt = nc_fid.variables[v_map['melt']][m-1, yi1::sub, ::sub] # time, j, i
            time = nc_fid.variables[v_map['time']][m-1]

        melt = np.ma.masked_where((melt==1e20), melt)  
        
        melt_time[m-1, :, :] = melt_time[m-1, :, :] + melt
        date[i] = cftime.num2date(time, ref, calendar='360_day')
        print(date[i])

melt_time = melt_time / len(yr_uni)
melt_time = melt_time.filled(-1e20)
np.savez(out_file + 'melt_mn_' + str_year + '.npz', melt_map=melt_time, lat=lat, lon=lon)
