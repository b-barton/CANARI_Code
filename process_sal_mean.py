...
sal_time = np.ma.zeros((12, lon.shape[0], lon.shape[1]))
for m in range(1, 13):
    for y in yr_uni:
        ...
        sal_time[m-1, :, :] = sal_time[m-1, :, :] + sal[0, :, :] # surface salinity
        date[i] = cftime.num2date(time, ref, calendar='360_day')[0]
        print(date[i])

sal_time = sal_time / len(yr_uni)
sal_time = sal_time.filled(-1e20)
np.savez(out_file + 'sal_mn_' + str_year + '.npz', sal_map=sal_time, lat=lat, lon=lon)
