...
fresh_time = np.ma.zeros((12, lon.shape[0], lon.shape[1]))
for m in range(1, 13):
    for y in yr_uni:
        ...
        fresh_time[m-1, :, :] = fresh_time[m-1, :, :] + np.ma.sum(calc_fresh(sal) * e3t, axis=0) # vertically integrated
        date[i] = cftime.num2date(time, ref, calendar='360_day')[0]
        print(date[i])

fresh_time = fresh_time / len(yr_uni)
fresh_time = fresh_time.filled(-1e20)
np.savez(out_file + 'fresh_mn_' + str_year + '.npz', fresh_map=fresh_time, lat=lat, lon=lon)
