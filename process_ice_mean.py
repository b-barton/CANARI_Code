...
if 1:
    st_date = dt.datetime(1980, 1, 1)
    en_date = dt.datetime(2010, 1, 1)
else:
...
date_from = np.zeros((len(flist_sic)), dtype=object)
date_to = np.zeros((len(flist_sic)), dtype=object)
for i in range(len(flist_sic)):
    part = flist_sic[i].split('_')[-1].split('.')[0].split('-')
...
sic_time = np.ma.zeros((12, lon.shape[0], lon.shape[1]))
sit_time = np.ma.zeros((12, lon.shape[0], lon.shape[1]))
for m in range(1, 13):
    for y in yr_uni:
        ind = np.nonzero(yr == y)[0][0] + date_use[0]
        with nc.Dataset(flist_sic[ind], 'r') as nc_fid:
            sic = nc_fid.variables[v_map['siconc']][m-1, yi1::sub, ::sub] # time, lev, j, i
            time = nc_fid.variables[v_map['time']][m-1]
        with nc.Dataset(flist_sit[ind], 'r') as nc_fid:
            sit = nc_fid.variables[v_map['sithick']][m-1, yi1::sub, ::sub]
    
        sic = np.ma.masked_where((sic==1e20), sic)
        sit = np.ma.masked_where((sit==1e20), sit)       

        sic_time[m-1, :, :] = sic_time[m-1, :, :] + sic 
        sit_time[m-1, :, :] = sit_time[m-1, :, :] + sit 
        date[i] = cftime.num2date(time, ref, calendar='360_day')
        print(date[i])

sic_time = sic_time / len(yr_uni)
sit_time = sit_time / len(yr_uni)
sic_time = sic_time.filled(-1e20)
sit_time = sit_time.filled(-1e20)
np.savez(out_file + 'ice_mn_' + str_year + '.npz', sic_map=sic_time, sit_map=sit_time, lat=lat, lon=lon)
